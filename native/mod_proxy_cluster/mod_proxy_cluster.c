...
static void* APR_THREAD_FUNC execute_watchdog(apr_thread_t *thread, void *data) {
    ...
    if (s) {
        ...
        /* If threads are used, then we need to get the original pool - parent of targs_pool */
        proxy_cluster_watchdog_func(s, thread ? apr_pool_parent_get(pool) : pool);
                                         ...
                                         apr_pool_t *pool)
{
    ...
    watchdog_thread_args_t targs = { .server = s, .pool = pool };
#if MC_USE_THREADS
    ...
    switch (state) {
        case AP_WATCHDOG_STATE_STARTING:
            ...
            if (mc_thread_pool_size && mc_thread_pool == NULL) {
                res = apr_thread_pool_create(&mc_thread_pool, mc_thread_pool_size, mc_thread_pool_size, pool);
                if (res != APR_SUCCESS) {
                    ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "apr_thread_pool_create failed");
                    mc_thread_pool = NULL;
                } else {
                    ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "apr_thread_pool_create successful");
                }
            } else {
                mc_thread_pool = NULL;
...
#if MC_USE_THREADS
            if (mc_thread_pool) {
                apr_pool_create(&targs_pool, pool);
                ...
                /* Thread alloc failed, so run it without threads!
