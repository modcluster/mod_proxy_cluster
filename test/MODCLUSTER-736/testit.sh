...
singlecycle() {
    ...
    while true
    do
        curl -s -m 20 http://localhost:8000/mod_cluster_manager | grep Node | grep tomcat$1 > /dev/null
        if [ $? -eq 0 ]; then
            break
        fi
        if [ $i -gt 300 ]; then
    ...
    while true
    do
        curl -s -m 20 http://localhost:8000/mod_cluster_manager | grep /tomcat$1 > /dev/null
        if [ $? -eq 0 ]; then
            break
        fi
        if [ $i -gt 300 ]; then
