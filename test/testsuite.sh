...
do
    IMG="$IMG:$tomcat_version" tomcat_create $tomcat_version > /dev/null 2>&1 || exit 3
    IMG="$IMG:$tomcat_version" run_test basetests.sh "Basic tests with tomcat $tomcat_version"
    res=$(expr $res + $?)
done
